...
const HEROES = [
  // TERRAN
  { id: 'raynor', name: 'Jim Raynor', faction: 'terran', hp: 4, skills: [
    { id: 'command', name: '号召', desc: 'Terran allies can play 闪 for you when damaged' },
    { id: 'guerrilla', name: '游击', desc: 'Can counter-杀 when attacked outside your turn' }
  ]},
  { id: 'tychus', name: 'Tychus', faction: 'terran', hp: 4, skills: [
    { id: 'suppression', name: '压制火力', desc: 'Can play 2 杀 per turn' },
    { id: 'desperado', name: '亡命', desc: '杀 damage+1 when hp=1' }
  ]},
  { id: 'banshee', name: 'Banshee Pilot', faction: 'terran', hp: 3, skills: [
    { id: 'cloak', name: '隐形', desc: 'Immune to first trick card each turn' },
    { id: 'airstrike', name: '空袭', desc: '杀 ignores distance' }
  ]},
  { id: 'medic', name: 'Medic', faction: 'terran', hp: 3, skills: [
    { id: 'firstaid', name: '急救', desc: 'Discard red card to heal any player 1hp' },
    { id: 'nanorepair', name: '纳米修复', desc: 'Draw 1 when healed' }
  ]},
  { id: 'nova', name: 'Nova', faction: 'terran', hp: 3, skills: [
    { id: 'nuke', name: '核弹定点', desc: 'Discard 2 red cards, deal 2 damage to one target' },
    { id: 'stealth', name: '潜行', desc: 'Cannot be targeted by 杀 from distance>1 outside your turn' }
  ]},
  { id: 'swann', name: 'Swann', faction: 'terran', hp: 4, skills: [
    { id: 'repair', name: '修理', desc: 'Discard equipment card to heal 1hp' },
    { id: 'armament', name: '军备升级', desc: 'Draw 2 when equipping weapon' }
  ]},
  // PROTOSS
  { id: 'zeratul', name: 'Zeratul', faction: 'protoss', hp: 4, skills: [
    { id: 'shadowstrike', name: '暗影突袭', desc: '杀 cannot be dodged, once per turn' },
    { id: 'voidcloak', name: '虚空隐匿', desc: 'During discard, set aside 1 card face down, use as any basic card next turn' }
  ]},
  { id: 'artanis', name: 'Artanis', faction: 'protoss', hp: 4, skills: [
    { id: 'glory', name: '光辉统帅', desc: "Protoss allies' 闪 can be used as 杀" },
    { id: 'warp', name: '折跃', desc: 'Swap seat position with any player once per turn' }
  ]},
  { id: 'highTemplar', name: 'High Templar', faction: 'protoss', hp: 3, skills: [
    { id: 'psistorm', name: '灵能风暴', desc: 'Discard 2 cards, deal 1 damage to ALL others' },
    { id: 'feedback', name: '反馈', desc: 'When damaged by trick card, deal 1 damage back to source' }
  ]},
  { id: 'sentry', name: 'Sentry', faction: 'protoss', hp: 3, skills: [
    { id: 'forcefield', name: '力场', desc: 'Discard 1 black card, one player cannot be targeted by 杀 this turn' },
    { id: 'hallucination', name: '幻象', desc: 'May skip draw phase to generate 2 幻象 cards that act as 闪' }
  ]},
  { id: 'tassadar', name: 'Tassadar', faction: 'protoss', hp: 3, skills: [
    { id: 'illusion', name: '幻象投影', desc: 'Discard 2 cards to negate damage' },
    { id: 'sacrifice', name: '牺牲', desc: 'When you die, fully heal one player' }
  ]},
  { id: 'fenix', name: 'Fenix', faction: 'protoss', hp: 4, skills: [
    { id: 'immortal', name: '不灭', desc: 'First time entering dying state, auto recover to 2hp, once per game' },
    { id: 'warrior', name: '勇士之魂', desc: '杀 damage+1 when hp<2' }
  ]},
  // ZERG
  { id: 'kerrigan', name: 'Kerrigan', faction: 'zerg', hp: 4, skills: [
    { id: 'swarmwill', name: '虫群意志', desc: 'When Zerg ally dies, recover 1hp and draw 2' },
    { id: 'parasite', name: '寄生', desc: "After dealing damage, view target's hand and take 1 card" }
  ]},
  { id: 'zergling', name: 'Zergling Alpha', faction: 'zerg', hp: 3, skills: [
    { id: 'rush', name: '狂涌', desc: 'Can play unlimited 杀 per turn' },
    { id: 'evolve', name: '蜕变', desc: 'Draw 2 extra when hp=1' }
  ]},
  { id: 'infestor', name: 'Infestor', faction: 'zerg', hp: 3, skills: [
    { id: 'infest', name: '感染', desc: "Discard 1 black card, control target's next card target" },
    { id: 'fungal', name: '真菌生长', desc: "Discard 1 card outside turn to skip target's play phase" }
  ]},
  { id: 'overlord', name: 'Overlord', faction: 'zerg', hp: 4, skills: [
    { id: 'detect', name: '侦测', desc: "May draw 1 less to view 2 players' hands" },
    { id: 'transport', name: '运输', desc: 'Transfer equipment to another player' }
  ]},
  { id: 'ultralisk', name: 'Ultralisk', faction: 'zerg', hp: 5, skills: [
    { id: 'heavyarmor', name: '重甲', desc: 'All damage received -1, min 1' },
    { id: 'trample', name: '践踏', desc: 'After 杀 hits, deal 1 to adjacent player' }
  ]},
  { id: 'mutalisk', name: 'Mutalisk', faction: 'zerg', hp: 3, skills: [
    { id: 'bounce', name: '弹射', desc: 'After 杀 hits, deal 1 to adjacent player' },
    { id: 'agility', name: '敏捷', desc: 'Draw 1 when playing 闪 outside your turn' }
  ]},
];
...
function buildDeck() {
  ...
  for (const [suit, num] of strikeDistrib) {
    add('杀', 'basic', 'strike', suit, num, '高斯步枪射击 - Deal 1 damage to target in range');
  }
  ...
  for (const [suit, num] of dodgeDistrib) {
    add('闪', 'basic', 'dodge', suit, num, '护盾闪避 - Negate a 杀');
  }
  ...
  for (const [suit, num] of peachDistrib) {
    add('桃', 'basic', 'peach', suit, num, '能量补给 - Recover 1 HP');
  }

  // Trick cards
  // 虫群入侵 (Barbarian Invasion) - 3
  add('虫群入侵', 'trick', 'barbarian', S, 7, 'All others must play 杀 or lose 1hp');
  add('虫群入侵', 'trick', 'barbarian', S, 13, 'All others must play 杀 or lose 1hp');
  add('虫群入侵', 'trick', 'barbarian', C, 7, 'All others must play 杀 or lose 1hp');

  // 战列巡航舰轰炸 (Arrow Rain) - 1
  add('战列巡航舰轰炸', 'trick', 'arrowrain', H, 1, 'All others must play 闪 or lose 1hp');

  // 星际联盟 (Peach Garden) - 1
  add('星际联盟', 'trick', 'peachgarden', H, 1, 'All players recover 1hp');

  // 矿物开采 (Something for Nothing) - 4
  add('矿物开采', 'trick', 'draw2', H, 7, 'Draw 2 cards');
  add('矿物开采', 'trick', 'draw2', H, 8, 'Draw 2 cards');
  add('矿物开采', 'trick', 'draw2', H, 9, 'Draw 2 cards');
  add('矿物开采', 'trick', 'draw2', H, 11, 'Draw 2 cards');

  // 一对一单挑 (Duel) - 3
  add('一对一单挑', 'trick', 'duel', S, 1, 'Duel target: take turns playing 杀');
  add('一对一单挑', 'trick', 'duel', C, 1, 'Duel target: take turns playing 杀');
  add('一对一单挑', 'trick', 'duel', D, 1, 'Duel target: take turns playing 杀');

  // 掠夺协议 (Snatch) - 5
  add('掠夺协议', 'trick', 'snatch', S, 3, 'Take 1 card from target (distance 1)');
  add('掠夺协议', 'trick', 'snatch', S, 4, 'Take 1 card from target (distance 1)');
  add('掠夺协议', 'trick', 'snatch', S, 11, 'Take 1 card from target (distance 1)');
  add('掠夺协议', 'trick', 'snatch', D, 3, 'Take 1 card from target (distance 1)');
  add('掠夺协议', 'trick', 'snatch', D, 4, 'Take 1 card from target (distance 1)');

  // EMP打击 (Dismantlement) - 6
  add('EMP打击', 'trick', 'dismantle', S, 3, "Discard 1 of target's cards");
  add('EMP打击', 'trick', 'dismantle', S, 4, "Discard 1 of target's cards");
  add('EMP打击', 'trick', 'dismantle', S, 12, "Discard 1 of target's cards");
  add('EMP打击', 'trick', 'dismantle', C, 3, "Discard 1 of target's cards");
  add('EMP打击', 'trick', 'dismantle', C, 4, "Discard 1 of target's cards");
  add('EMP打击', 'trick', 'dismantle', H, 12, "Discard 1 of target's cards");

  // 电磁干扰 (Indulgence) - 3 delayed trick
  add('电磁干扰', 'trick', 'indulgence', S, 6, 'Delayed: skip play phase if judgement is not heart');
  add('电磁干扰', 'trick', 'indulgence', C, 6, 'Delayed: skip play phase if judgement is not heart');
  add('电磁干扰', 'trick', 'indulgence', H, 6, 'Delayed: skip play phase if judgement is not heart');

  // 反制矩阵 (Nullification) - 4
  add('反制矩阵', 'trick', 'nullify', S, 11, 'Counter a trick card');
  add('反制矩阵', 'trick', 'nullify', C, 12, 'Counter a trick card');
  add('反制矩阵', 'trick', 'nullify', C, 13, 'Counter a trick card');
  add('反制矩阵', 'trick', 'nullify', D, 12, 'Counter a trick card');

  // 雇佣兵合同 (Borrow Sword) - 1
  add('雇佣兵合同', 'trick', 'borrowsword', C, 12, 'Force player to 杀 another or give you their weapon');

  // 核弹打击 (Fire Attack) - 3
  add('核弹打击', 'trick', 'fireattack', H, 2, 'Target reveals hand, discard matching suit to deal 1 fire damage');
  add('核弹打击', 'trick', 'fireattack', H, 3, 'Target reveals hand, discard matching suit to deal 1 fire damage');
  add('核弹打击', 'trick', 'fireattack', D, 12, 'Target reveals hand, discard matching suit to deal 1 fire damage');

  // 离子风暴 (Lightning) - 2 delayed trick
  add('离子风暴', 'trick', 'lightning', S, 1, 'Delayed: 3 damage if judgement is spade 2-9, passes to next');
  add('离子风暴', 'trick', 'lightning', H, 12, 'Delayed: 3 damage if judgement is spade 2-9, passes to next');

  // Equipment - Weapons
  add('加特林机炮', 'equipment', 'weapon', D, 1, 'Range 1, unlimited 杀 per turn');
  add('灵能刀刃', 'equipment', 'weapon', S, 6, 'Range 3, chain 杀 on dodge');
  add('C-14穿甲枪', 'equipment', 'weapon', S, 12, 'Range 3, discard 2 cards as 杀');
  add('攻城坦克炮', 'equipment', 'weapon', D, 5, 'Range 3, discard 2 to force hit on dodge');
  add('航母拦截机', 'equipment', 'weapon', S, 5, 'Range 4, if last card 杀 can target 3');
  add('幽灵狙击枪', 'equipment', 'weapon', H, 5, 'Range 5, on hit remove target mount');

  // Equipment - Armor
  add('护盾电池', 'equipment', 'armor', S, 2, 'Judge: heart/diamond = auto dodge');
  add('护盾电池', 'equipment', 'armor', C, 2, 'Judge: heart/diamond = auto dodge');
  add('相位护甲', 'equipment', 'armor', C, 2, 'Immune to black 杀');
  add('再生甲虫', 'equipment', 'armor', C, 1, 'Max 1 damage, heal 1 on unequip');
  add('生化护甲', 'equipment', 'armor', S, 2, 'Immune to normal 杀 and 南蛮, fire damage+1');

  // Equipment - Mounts
  // +1 horse (defensive) - 3 cards
  add('传送棱镜', 'equipment', 'horse_plus', H, 13, 'Others +1 distance to you');
  add('传送棱镜', 'equipment', 'horse_plus', S, 5, 'Others +1 distance to you');
  add('传送棱镜', 'equipment', 'horse_plus', C, 5, 'Others +1 distance to you');

  // -1 horse (offensive) - 3 cards
  add('超时空加速器', 'equipment', 'horse_minus', H, 5, 'You -1 distance to others');
  add('超时空加速器', 'equipment', 'horse_minus', D, 13, 'You -1 distance to others');
  add('超时空加速器', 'equipment', 'horse_minus', S, 13, 'You -1 distance to others');
...
class GameEngine {
  constructor(room) {
    ...
    this.pendingResponse = null;  // { type, fromPlayer, targetPlayer, card, callback }
  ...
  broadcastLog(text) {
    this.log.push(text);
    this.room.broadcast({ type: 'log', text });
  }
  ...
  playPhase(player) {
    ...
    if (player.isAI) {
      setTimeout(() => aiPlayPhase(this, player), 1000 + Math.random() * 1000);
    }
  ...
  discardPhase(player) {
    ...
    if (player.isAI) {
      setTimeout(() => aiDiscardPhase(this, player, toDiscard), 1000);
    } else {
  ...
  playStrike(player, card, target) {
    ...
    if (player.heroId === 'zeratul' && player.strikeCount === 1) {
      let damage = 1;
      if (player.heroId === 'tychus' && player.hp === 1) damage++;
    ...
    let damage = 1;
    if (player.heroId === 'tychus' && player.hp === 1) damage++; // 亡命
  ...
  requestDodge(attacker, target, strikeCard, damage, factionCountered) {
    ...
    target.send({
      type: 'requestResponse', requestType: 'dodge',
      fromPlayer: attacker.id,
      card: { id: strikeCard.id, name: strikeCard.name },
      factionCountered
    });

    if (target.isAI) {
      setTimeout(() => aiRespondDodge(this, target, factionCountered), 1000 + Math.random() * 500);
    }
  }
  ...
  strikeHit(attacker, target, strikeCard, damage) {
    ...
    // Trample: ultralisk / mutalisk bounce
    if ((attacker.heroId === 'ultralisk' || attacker.heroId === 'mutalisk') && target.alive) {
      const adjacent = this.adjacentPlayers(target);
      for (const adj of adjacent) {
        if (adj.id !== attacker.id && adj.alive) {
          this.broadcastLog(`${attacker.name} 的${attacker.heroId === 'ultralisk' ? '践踏' : '弹射'}对 ${adj.name} 造成1点伤害。`);
          this.dealDamage(attacker, adj, 1, 'trample');
          break;
        }
      }
    }
  ...
  resolveAOE(source, targets, idx, responseType) {
    ...
    target.send({
      type: 'requestResponse', requestType: reqType,
      fromPlayer: source.id, card: null
    });

    if (target.isAI) {
      setTimeout(() => aiRespondAOE(this, target, neededSubtype), 800 + Math.random() * 500);
    }
  }
  ...
  resolveDuel(source, target, currentResponder) {
    ...
    currentResponder.send({
      type: 'requestResponse', requestType: 'duel_strike',
      fromPlayer: other.id, card: null
    });

    if (currentResponder.isAI) {
      setTimeout(() => aiRespondDuel(this, currentResponder), 800);
    }
  }
  ...
  requestPeach(dyingPlayer, killer) {
    ...
    if (!dyingPlayer.isAI) {
      ...
      dyingPlayer.send({
        type: 'requestResponse', requestType: 'peach',
        fromPlayer: null, card: null
      });
      return;
    }
  ...
  handleDeath(player, killer) {
    ...
    this.room.broadcast({ type: 'playerDied', playerId: player.id, identity: player.identity });
    this.broadcastLog(`${player.name} 阵亡了！身份是 ${player.identity}。`);
  ...
  endGame(winners, winIdentity) {
    ...
    this.broadcastLog(`游戏结束！${winIdentity} 阵营获胜！`);
  }
...
function aiPlayPhase(game, player) {
  ...
  function executeActions(idx) {
    ...
    setTimeout(() => executeActions(idx + 1), 800 + Math.random() * 400);
  }
  ...
  switch (msg.type) {
    ...
    case 'joinRoom': {
      ...
      if (room.game && room.game.gameStarted) {
        sendTo(ws, { type: 'error', message: '游戏已经开始。' });
        return;
      }
    ...
    case 'addAI': {
      ...
      const aiNames = ['AI-Marine', 'AI-Zealot', 'AI-Hydra', 'AI-Ghost', 'AI-Stalker', 'AI-Roach', 'AI-Viking'];
