...
class Room {
  constructor(id, creatorId, isPublic = true) {
    this.id = id;
...
function handleMessage(ws, playerId, data) {
  ...
  switch (msg.type) {
    case 'createRoom': {
      ...
      const isPublic = msg.isPublic !== false; // Default to true
      const roomId = generateRoomId();
      const room = new Room(roomId, playerId, isPublic);
      rooms.set(roomId, room);
      const player = new Player(playerId, playerName, ws);
      room.addPlayer(player);
      playerRoomMap.set(playerId, roomId);
      sendTo(ws, { type: 'roomCreated', roomId, playerId, isPublic });
    ...
    case 'getRoomList': {
      ...
      for (const [roomId, room] of rooms) {
        if (room.isPublic && (!room.game || !room.game.gameStarted) && room.players.length < 8) {
          publicRooms.push({
            roomId: room.isPublic ? roomId : null, // 只对公开房间显示房间号
            playerCount: room.players.length,
            maxPlayers: 8,
            creator: room.players.find(p => p.id === room.creatorId)?.name || 'Unknown'
          });
        }
      }
      sendTo(ws, { type: 'roomList', rooms: publicRooms });
      break;
    }

    case 'toggleRoomPrivacy': {
      const roomId = playerRoomMap.get(playerId);
      if (!roomId) return;
      const room = rooms.get(roomId);
      if (!room) return;
      if (room.creatorId !== playerId) {
        sendTo(ws, { type: 'error', message: '只有房主可以切换房间状态' });
        return;
      }
      room.isPublic = !room.isPublic;
      // 通知房间内所有玩家
      room.broadcast({
        type: 'roomPrivacyChanged',
